{{-- filepath: c:\laragon\www\SertijabPelni\resources\views\kelolaKapal\edit.blade.php --}}
@extends('layouts.app')

@section('title', 'Edit Kapal - SertijabPELNI')
    ...
    <div class="page-header">
        <div class="header-content">
            <div class="header-left">
                ...
                <p class="page-subtitle">Perbarui informasi kapal</p>
            </div>
        ...
        <div class="col-xl-8 col-lg-10">
            <div class="form-card">
                <div class="form-header">
                    ...
                    <p class="form-subtitle">Lengkapi semua data kapal dengan benar <span class="text-danger">(semua field wajib diisi)</span></p>
                </div>
                
                <form id="kapalForm" action="{{ route('kapal.update', $kapal->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-body">
                        <div class="row">
                            ...
                            <div class="col-lg-6">
                                <div class="form-group mb-3">
                                    <label for="tipe_pax" class="form-label required">Kapasitas Penumpang (PAX)</label>
                                    <input type="number" class="form-control @error('tipe_pax') is-invalid @enderror" 
                                           id="tipe_pax" name="tipe_pax" 
                                           value="{{ old('tipe_pax', $kapal->tipe_pax) }}"
                                           placeholder="Contoh: 1500" min="0" max="10000" required>
                                    @error('tipe_pax')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                            ...
                            <div class="col-lg-6">
                                <div class="form-group mb-3">
                                    <label for="home_base" class="form-label required">Home Base</label>
                                    <select class="form-select select2 @error('home_base') is-invalid @enderror" 
                                            id="home_base" name="home_base" required>
                                        <option value="">-- Pilih atau ketik home base --</option>
                                        @foreach($homeBaseList as $homeBase)
                                            <option value="{{ $homeBase }}" {{ old('home_base', $kapal->home_base) == $homeBase ? 'selected' : '' }}>
                                                {{ $homeBase }}
                                            </option>
                                        @endforeach
                                    </select>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-body text-center py-4">
                ...
                <p class="mb-4">Data kapal berhasil diperbarui</p>
...
/* Form label required style - buat lebih mencolok */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function validateForm() {
        let isValid = true;
        const allFields = form.querySelectorAll('input, select'); // Validasi semua field
        ...
        if (tipePaxInput.value.trim()) {
            if (tipePaxValue < 0 || tipePaxValue > 10000) {
                showValidationError(tipePaxInput, 'Kapasitas harus antara 0 - 10.000');
                isValid = false;
            }
        }
        
        // Validate home base has valid selection
        const homeBaseValue = $('#home_base').val();
        if (homeBaseValue) {
            const isHomeBaseValid = $('#home_base option').filter(function() {
                return $(this).val() === homeBaseValue;
            }).length > 0;
